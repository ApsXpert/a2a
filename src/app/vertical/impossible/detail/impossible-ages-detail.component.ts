/**
 * This component is for Comprehensive periodontal exam detail
 */
...
@Component({
    ...
    selector: 'impossible-ages-detail',
    templateUrl: 'impossible-ages-detail.component.html'
})
export class ImpossibleAgesDetailComponent implements OnInit {
    ...
    constructor(private vdcService: VerticalDetailService, private appDataService: AppDataService,
                private logout: LogoutService, private router: Router, private centeralHttpService: CenteralHttpService) {
    }
    ...
    public pageChanged(pageNumber: any) {
        ...
        this.centeralHttpService.post(URL.IMPOSSIBLE.DOCTOR_REPORTS, this.vdcService.vdRequest)
            .subscribe(
                (data: any) => {
                    if (+data.statusCode == 401 || data.success == false) {
                        this.logout.doLogout(APP.MESSAGES.SESSION_EXPIRED);
                    } else {
                        this.vdcService.vdcData.doctorReports = data.data;
            ...
            .subscribe(
                (data: any) => {
                    if (+data.statusCode == 401 || data.success == false) {
                        ...
                        this.vdcService.vdcData.doctorReports = data.data
                        this.vdcService.vdcData.count = data.count;
                    }
